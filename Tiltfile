...
cleanup_command = """
  ...
  helm template {3} -f {1} -s templates/data-migration-job.yaml {2} | kubectl apply -n {0} -f -;
  sleep 10;
  ...
  helm template {3} -f {1} --set data_seed.enabled=true -s templates/data-seed-job.yaml {2} | kubectl apply -n {0} -f -;
  sleep 10;
